...
class TestGenerator:
    def test_index_has_expected_count(self):
        index = load_index()
        assert len(index) == EXPECTED_COUNT, f"expected {EXPECTED_COUNT} scenarios, found {len(index)}"
    ...
    def test_generator_exit_zero(self):
        ...
        assert proc.returncode == 0, f"generate_dataset.py failed:\n{proc.stderr}"
...
class TestIncidentIntegrity:
    @pytest.mark.parametrize("iid", [f"INC-{i:03d}" for i in range(1, EXPECTED_COUNT + 1)])
...
class TestGoldIntegrity:
    def test_gold_count_matches_index(self):
        ...
        assert incident_ids == gold_ids, (
            f"mismatch: only-in-incidents={incident_ids - gold_ids}, only-in-gold={gold_ids - incident_ids}"
        )
...
class TestNoLeakage:
    ...
    def test_no_gold_keys_in_meta(self, iid):
        ...
        for forbidden in ["root_cause", "recommended_remediation"]:
            assert forbidden not in meta_text, f"{iid}: '{forbidden}' leaked into meta.json"

    @pytest.mark.parametrize("iid", [f"INC-{i:03d}" for i in range(1, EXPECTED_COUNT + 1)])
    def test_no_root_cause_in_meta(self, iid):
        meta = load_incident_files(iid)["meta"]
        meta_text = json.dumps(meta).lower()
        assert "root_cause" not in meta_text, f"{iid}: 'root_cause' leaked into meta.json"

    def test_gold_dir_outside_incidents(self):
        """Gold answers must never be under data/incidents/."""
        for p in GOLD_DIR.glob("INC-*.json"):
            resolved = p.resolve()
            assert "incidents" not in str(resolved).lower(), f"{p.name} is under data/incidents/ — eval leakage!"
...
class TestFalseAlarm:
    ...
    def test_incident_019_no_error_warn_logs(self):
        ...
        assert "level=ERROR" not in logs, "INC-019 must not contain ERROR log lines"
        assert "level=WARN" not in logs, "INC-019 must not contain WARN log lines"
...
class TestRecurrencePair:
    def test_incident_020_recurrence_of_002(self):
        ...
        assert g020.get("recurrence_of") == "INC-002", "INC-020 should reference INC-002 as recurrence_of"
        ...
        for entry in index:
            ...
            if rec:
                assert rec in incident_ids, (
                    f"{entry['incident_id']}: recurrence_of='{rec}' does not match any known incident id"
                )
...
class TestDistractorPairs:
    def test_incident_005_has_distractor_note(self):
        g = load_gold("INC-005")
        assert "distractor_note" in g and g["distractor_note"], "INC-005 should have a distractor_note"

    def test_incident_007_has_distractor_note(self):
        g = load_gold("INC-007")
        assert "distractor_note" in g and g["distractor_note"], "INC-007 should have a distractor_note"
...
class TestValidator:
    def test_validator_exit_zero(self):
        ...
        assert proc.returncode == 0, f"validate_dataset.py failed:\n{proc.stderr}\n{proc.stdout}"
...
class TestGoldSetMd:
    def test_gold_set_md_exists(self):
        assert (ROOT / "eval" / "gold_set.md").exists(), "eval/gold_set.md missing — run eval/generate_gold_set_md.py"

    def test_gold_set_md_contains_all_ids(self):
        content = (ROOT / "eval" / "gold_set.md").read_text()
        for i in range(1, EXPECTED_COUNT + 1):
            assert f"INC-{i:03d}" in content, f"gold_set.md missing INC-{i:03d}"

    def test_gold_set_md_mentions_recurrence_pair(self):
        content = (ROOT / "eval" / "gold_set.md").read_text()
        assert "INC-002" in content and "INC-020" in content, "gold_set.md should mention the recurrence pair"

    def test_gold_set_md_mentions_false_alarm(self):
        content = (ROOT / "eval" / "gold_set.md").read_text()
        assert "INC-019" in content, "gold_set.md should mention INC-019"
